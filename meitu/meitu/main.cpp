...
double rotating_calipers_min(int n)
{
    int j=1,k=0;
    double ans=1e9;
    for(int i=0;i<n;i++)
    {
        ...
        while(j!=i&&k!=i)
        {
			double temp=fabs(cross(stack[i],stack[j],stack[k]));
			if(temp>-0.0000001 && temp< 0.000001)
			{;}
			else
			{
				ans=min(ans,temp);
...
int main()
{
    ...
    while(~scanf("%d",&n))
    {
        ...
        printf("%.1lf %.1lf\n",rotating_calipers_min(top),rotating_calipers(top));
    }
    return 0;
}